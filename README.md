...
We conduct an auction between all the LLMs, and we chose an LLM such that our reward is maximized(greedy approach). In each step we compute the expected reward using the UCB Algorithm, and chose the best LLM with Reverse Myerson Auctions(refer of the paper attached).
